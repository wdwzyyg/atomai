...
class datatransform:
    ...
    Args:
        ...
        **custom_transform (Callable):
            ...
            transformed arrays
        **shift (bool or list ot tuple):
            bean random shift. You can pass min and max values as a list/tuple
            (Default [min, max] range: [0, 20])
        **flyback (bool or list ot tuple):
            bean flyback error. You can pass min and max values as a list/tuple
            (Default [min, max] range: [0, 25])
        **rotation (bool):
    ...
    def __init__(self,
        ...
        if self.shift is True:
            self.shift = [0, 20]        
        self.flyback = kwargs.get('flyback')
        if self.flyback is True:
            self.flyback = [0, 25]
